...
def token_repo_mock():
    repo = Mock()
    repo.is_revoked = Mock(return_value=False)
    repo.revoke_token = Mock()
    return repo
...
async def test_verify_returns_token_payload(jwt_manager, token_repo_mock):
    ...
    payload = await jwt_manager._verify(token)

    assert isinstance(payload, TokenPayload)
    assert payload.sub == "42"
    token_repo_mock.is_revoked.assert_called_once_with(payload.jti)


async def test_verify_raises_on_invalid_signature(jwt_manager, token_repo_mock):
    token = jwt_manager.issue_access(user_id=1)

    bad_token = f"{token}broken"

    with pytest.raises(TokenVerifyError) as exc_info:
        await jwt_manager._verify(bad_token)

    assert "Invalid or expired token" in str(exc_info.value)
    token_repo_mock.is_revoked.assert_not_called()
...
async def test_verify_raises_on_revoked_token(jwt_manager, token_repo_mock):
    ...
    with pytest.raises(TokenVerifyError) as exc_info:
        await jwt_manager._verify(token)

    assert "Revoked token" in str(exc_info.value)
    token_repo_mock.is_revoked.assert_called_once_with(jti)
...
async def test_revoke_calls_repo_with_jti_and_exp(jwt_manager, token_repo_mock):
    ...
    token_repo_mock.revoke_token.assert_called_once_with(jti, exp)
